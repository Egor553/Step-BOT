...
import { User, Map, CheckCircle2, Briefcase, Phone, Plus, Star, ChevronLeft, Target, Gamepad2, Users, Trophy, Snowflake, Zap, Trash2, MessageSquare, HandHelping, Lightbulb, Clock, LogOut, Gift, Mail } from 'lucide-react';
        ...
        <div className="app-container">
            ...
            <nav className="nav-bar">
                ...
                <div className={`nav-item ${tab === 'profile' ? 'active' : ''}`} onClick={() => setTab('profile')}>
                    <User size={22} /> <span>Кабинет</span>
                </div>
            </nav>
        </div>
    );
...
const ProfileView = ({ user, onSave, onLogout }: any) => {
    ...
    return (
        <div style={{ paddingBottom: 100 }}>
            ...
            {/* Блок Помощи по Навигации */}
            <div style={{ marginTop: 24, padding: '0 24px' }}>
                <h3 style={{ fontSize: 18, fontWeight: 800, marginBottom: 16 }}>🗺️ Навигация</h3>
                <div className="card" style={{ padding: 20 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        <div style={{ display: 'flex', gap: 12 }}>
                            <div style={{ minWidth: 24, height: 24, borderRadius: 8, background: 'rgba(37, 99, 235, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-blue)' }}><Target size={14} /></div>
                            <div>
                                <div style={{ fontWeight: 700, fontSize: 14 }}>Мой Путь</div>
                                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.4 }}>Ваши цели и шаги.</div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: 12 }}>
                            <div style={{ minWidth: 24, height: 24, borderRadius: 8, background: 'rgba(37, 99, 235, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-blue)' }}><Trophy size={14} /></div>
                            <div>
                                <div style={{ fontWeight: 700, fontSize: 14 }}>Отчеты</div>
                                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.4 }}>Статистика и графики.</div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: 12 }}>
                            <div style={{ minWidth: 24, height: 24, borderRadius: 8, background: 'rgba(37, 99, 235, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-blue)' }}><Lightbulb size={14} /></div>
                            <div>
                                <div style={{ fontWeight: 700, fontSize: 14 }}>Идеи</div>
                                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.4 }}>Предложить улучшение для бота.</div>
                            </div>
                        </div>
                    </div>
                </div>
...
const PathView = ({ user, onUpdate, onNavigateToTracker }: any) => {
    ...
    return (
        <div style={{ marginTop: 24, paddingBottom: 100 }}>
            ...
            {!selectedGoal && activeGoals.length > 0 && pathTab === 'active' && (
                <div style={{
                    ...
                    <span>Нажми на карточку цели, чтобы увидеть график прогресса и добавить новый шаг.</span>
                </div>
            )}
...
const TrackerView = ({ user, onUpdate }: any) => {
    ...
    return (
        <div className="card" style={{ marginTop: 24, padding: 24 }}>
            ...
            <p style={{ color: 'var(--text-secondary)', marginBottom: 24, fontSize: 14 }}>Опиши свой вызов. Ты можешь держать до 3 целей одновременно.</p>
        ...
        <div style={{ paddingBottom: 20 }}>
            ...
            {filteredSteps.length > 0 && (
                <div className="card">
                    ...
                    {filteredSteps.slice(0, 10).map((step: any, idx: number) => (
                        <div key={idx} style={{
                            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                            padding: '12px 0', borderBottom: idx < 9 ? '1px solid #f1f5f9' : 'none'
                        }}>
                            <div style={{ flex: 1, marginRight: 12 }}>
                                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 200 }}>
                                    {step.content || step.goalTitle}
                                </div>
                                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                                    {new Date(step.createdAt).toLocaleDateString()} • {step.goalTitle}
                                </div>
                            </div>
                            <div style={{
